...
def upsetsCalc(year, strVector, regions):
    ...
    upsetsListBottom = {'Year': year, regions[0]: 0, regions[1]: 0, regions[2]: 0, regions[3]: 0}
    ...
    for i in range (0, len(firstRoundRegions)):
        ...
        for j in range (0, len(secondRoundRegion)):
            ...
            if (secondRoundRegion[j] == max(seedOne, seedTwo) and (seedOne != 8 and seedTwo != 9)):
                if (max(seedOne, seedTwo) == 15 or max(seedOne, seedTwo) == 14 or max(seedOne, seedTwo) == 13):
                    numUpsetsBottom += 1
                numUpsetsForRegion += 1
        upsetsList[regions[i]] += numUpsetsForRegion
        upsetsListBottom[regions[i]] += numUpsetsBottom

    return upsetsListBottom
...
axs[0].hist(worstRegions, bins = [0,1,2,3,4,5])
axs[0].set_xticks([0,1,2,3,4])
axs[0].set_title("Max Upsets")
axs[1].hist(bestRegions, bins = [0,1,2])
